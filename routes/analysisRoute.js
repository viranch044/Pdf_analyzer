...
const error_check = (error) => {
    return (error == "Bad PDF header" || error == "%%EOF not found" || error == "Xref section not found" || error == 'EOF while looking for symbol \\"startxref\\"');
}
...
const suspiciousElementCheck = (data) => {
    ...
    for(var i = 0; i<data.peepdf_analysis.advanced.length; i++){
        var suspicious_element_object= data.peepdf_analysis.advanced[i].version_info.suspicious_elements;

        if(suspicious_element_object.actions != null || suspicious_element_object.triggers!=null || suspicious_element_object.urls!= null || 
            suspicious_element_object.elements.length > 0 || suspicious_element_object.js_vulns.length > 0) {
                sus_element_ = true;
                break;
            }
    }
    ...
    const getJavaScript = () => {
        return new Promise((resolve, reject) => {
            ...
            console.log("Heeee")
            var child = spawn("python", [`${path_peepdf}`, '-f', '-l','-s',  `${path_script}`,  `${fileName}`]);
            var resp = "";
            ...
            child.on('exit', (code, signal) => {   
                console.log('The UI automation script is completed and the tests are running on the remote machine');
                resolve(resp);
            });
        })
    }
...
router.get('/:file_name', (req, res, next) => {
    ...
    const callToolsPromise = () => {
        return new Promise((resolve, reject) => {
            ...
            // console.log(req)
            var child = spawn("python", [`${path_peepdf}`, '-f', '-j', `${fileName}`]);
            var resp = "";
    ...
    callToolsPromise().then(response => {
        const data = JSON.parse(response);
